...
exports.fileServices = class fileServices {
    ...
    static async updateByQuery(res, id, body) {
        ...
        try {
            return dataConversion(await FilesModel.update(
                {
                    caption,
                    title
                }, { where: { id } }))
    ...
    static async findByOriginalname(res, originalname) {
        try {
            return dataConversion(await FilesModel.findOne({ where: { originalname } }))
        } catch (error) {
            return res.status(500).json({
    ...
    static async create(res, id, body, originalname) {
        const { caption, title } = body;
        try {
            return dataConversion(await FilesModel.create(
                {
                    id,
                    caption,
